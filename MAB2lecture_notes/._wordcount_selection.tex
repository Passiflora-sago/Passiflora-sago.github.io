...
\NewDocumentCommand{\parfrac}{ s O{1} m m }
{
  \group_begin:
  \IfBooleanT {#1} { \cs_set:Npn \__parfrac_fractype: { \flatfrac } } % 星号用 \flatfrac
  \frac
  {
    \__parfrac_process_numerator:nn {#2}{#3} % 分子处理
  }
  {
    \__parfrac_process_denominator:nn {#2}{#4} % 分母处理（传入阶数）
  }
  \group_end:
}
...
\cs_new_protected:Nn \__parfrac_process_denominator:nn
{
  ...
  {
    \partial \l_tmpa_tl % 保留整体括号
    \int_compare:nNnT {#1} > {1} { ^{#1} } % 添加阶数
  }
  {
    \seq_set_split:Nnn \l_tmpa_seq { , } {#2}
    \int_compare:nNnTF {#1} = {1}
    {
      \seq_map_inline:Nn \l_tmpa_seq { \partial ##1 } % 单阶数
    }
    {
      \seq_map_inline:Nn \l_tmpa_seq { \partial ##1 ^{#1} } % 多变量各自加阶数
    }
  }
}
